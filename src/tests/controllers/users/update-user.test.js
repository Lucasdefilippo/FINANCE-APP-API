...
describe('UpdateUserController', () => {
    ...
    it('should return 400 when an invalid password is provid', async () => {
        const { sut } = makeSut()
    ...
    it('should return 400 when an unallowed field is provid', async () => {
        const { sut } = makeSut()
        ...
        const result = await sut.execute({
            ...
            body: {
                ...httpRequest.body,
                unallawed_field: 'unallowed_field',
            },
        })
    ...
    it('should return 400 if UpdateUserUseCase throws EmailAlreadyInUse', async () => {
        const { sut, updateUserUseCase } = makeSut()
        jest.spyOn(updateUserUseCase, 'execute').mockRejectedValueOnce(
            new EmailAlreadyInUse(faker.internet.email()),
        )
    ...
    it('should call UpdateUserUseCase with corrected params', async () => {
        const { sut, updateUserUseCase } = makeSut()
